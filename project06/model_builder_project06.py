...
DATASET_NAME: Final[str] = "diabetes_data"

# STEP 1. Pick the target variable we want to predict.
TARGET_COL: Final[str] = "diabetic"

OR_TARGET: Final[str] = "outcome"
...
# STEP 5. Define where the original dataset is
DATASET_PATH: Final[Path] = Path(__file__).parent / "diabetes_data.csv"
...
# === Section 2a. Load the Data ===


def load_data() -> pd.DataFrame:
    """Load the diabetes dataset and drop rows with missing required values."""
    LOG.info(f"Loading dataset: {DATASET_NAME}")

    df: pd.DataFrame = pd.read_csv(DATASET_PATH)
    LOG.info(f"Loaded: {df.shape[0]} rows, {df.shape[1]} columns")

    df["diabetic"] = df["outcome"].map({0: "Not Diabetic", 1: "Diabetic"})

    required: list[str] = [OR_TARGET, *FEATURE_COLS]
